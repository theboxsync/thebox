...
function AddCaptainModal({ show, handleClose, fetchCaptainData }) {
    ...
    onSubmit: async (values) => {
      ...
      try {
        const res = await axios.post(
          `${process.env.REACT_APP_ADMIN_API}/captain/addcaptain`,
          values,
          { withCredentials: true }
        );
        console.log("Captain Added:", res.data);
        fetchCaptainData();
        handleClose();
      } catch (err) {
  ...
  return (
    <Modal show={show} onHide={handleClose} backdrop="static" keyboard={false}>
      <Modal.Header>
        <Modal.Title>Add Captain</Modal.Title>
        <button
          type="button"
          className="btn-close"
          aria-label="Close"
